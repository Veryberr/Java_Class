...
//	데이터를 기억할 멤버 변수(필드)를 선언한다. => 클래스 내부에 선언되는 모든 메소드에서 사용 가능
//	일반 변수를 선언하면 변수에는 어떤 값이 들어있는지 모르지만(쓰레기) 클래스에서 선언하는 필드는
//	별도로 초기화를 하지 않아도 숫자는 0, 문자는 공백, boolean은 false, 클래스로 만든 객체(참조 변수)는
//	null로 자동 초기화 된다.
...
//	생성자를 선언하지 않으면 컴파일러가 자동으로 아무런 일도 하지 않는 생성자(기본 생성자)를 만들어 준다.
...
//		this(); // 재귀 호출 => 오류
//		this("없음", "없다니까는", "글쎄~~~~~", new Date(), 0); // 현재 클래스의 다른 생성자를 호출한다.(기본 생성자 초기화)
...
//		숫자 데이터 출력 서식 만들기
//		NumberFormat 클래스로 숫자 출력 서식 지정하기 => 미리 제작해서 제공되는 서식만 사용 가능
//		NumberFormat nf = NumberFormat.getInstance(); // 천 단위 구분 기호
