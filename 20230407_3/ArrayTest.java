...
//		배열이란 같은 이름으로 여러개의 기억장소를 만든다.
//		기억장소 구분은 인덱스(첨자)를 이용
		
//		배열을 선언만 함
//		****배열 형식****
//		자료형[] 배열이름 = new 자료형[첨자];  // 첨자로 지정한 크기만큼 배열 만듦
//		자료형 배열이름[] = new 자료형[첨자];
		
//		배열을 선언만하면 기본 초기화가 실행됨
//		숫자는 0, 실수는 0.0, 문자는 공백, boolean은 false, 
...
//		name 배열의 마지막 인덱스에 저장된 데이터가 str 변수에 저장된 후 처리되면 반복 종료
		for(String str : name) {
