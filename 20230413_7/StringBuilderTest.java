...
public class StringBuilderTest {
	...
	public static void main(String[] args) {
		...
		java = java + jsp;
		System.out.println(java);
//		String을 연결하면 기존의 String에 연결되는 것이 아니고 새로운 String 객체가 생성됨
		System.out.println(System.identityHashCode(java));
		
//		concat("문자열") : 문자열에 인수로 지정한 문자열을 이어 붙임
		java = java.concat(jsp);  // 새로운 String 객체 생성
		System.out.println(java);
		System.out.println(System.identityHashCode(java));
		
		String str = "";
		long start = System.currentTimeMillis(); // 현재 컴퓨터의 시간
...
//		StringBuilder 클래스는 내부적으로 가변적인 char[] 배열을 멤버 변수(필드)로 가짐
...
//		append() 메소드로 StringBuilder 클래스 객체에 문자열을 연결한다.
		...
		start = System.currentTimeMillis(); // 현재 컴퓨터의 시간
		for(int i=0; i<100000; i++) {
			str2.append("꽝");
		}
		System.out.println("실행 시간: " + (System.currentTimeMillis() - start));
...
//		멀티 스레드 프로그램은 StringBuffer를 사용하고 싱글(단일) 스레드 프로그램은
//		StringBuilder를 사용하는 것을 권장함
...
//		문자열을 """와 """사이에 입력하면 \n을 사용하지 않고도 줄바꿈되는 여러줄 문자열을 만들 수 있음
		String strBlock = """
						개울가에
						올챙이
						한마리
