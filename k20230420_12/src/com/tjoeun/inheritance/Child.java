...
//	Child 클래스는 Parent 클래스의 모든 필드와 메소드를 사용할 수 있다.(별도로 만들지 않아도)
...
//	정의하지 않은 필드(상속받은 필드)는 생성자를 만드는 대화상자에 표시되지 않는다.
	
//	부모 클래스에서 상속받은 필드의 접근 권한이 private 권한일 경우 자식 클래스에서
//	직접 접근할 수 없기 때문에 부모 클래스의 생성자를 호출해서 초기화시킨다.

	public Child(String name, boolean gender, int age, String nickname) {
		super(name, gender); // Parent(String name, boolean gender)가 실행됨
//		this.name = name; // 에러, 부모 클래스에서 private 권한으로 설정된 필드는 접근할 수 없다.
//		this.gender = gender; // 에러, 위와 동일
		this.age = age;
		this.nickname = nickname;
	}

		
//	부모 클래스에서 private 권한으로 설정된 모든 필드에 setter 메소드가 정의되어있다면
//	setter는 메소드이므로 public 권한으로 작성되었기 때문에 자식 클래스에서 실행할 수 있으므로 
//	부모 클래스에서 상속받은 private 권한의 필드를 setter를 이용해서 초기화시킬 수 있음
	/*
	public Child(String name, boolean gender, int age, String nickname) {
		setName(name);
		setGender(gender);
...
//		부모 클래스에서 상속받은 필드의 접근 권한이 protected 권한일 경우 자식 클래스에서 접근할 수
